...
public class CPHInline
{
    ...
    public bool Execute()
    {
        ...
        long balance = CPH.GetKickUserVar<long>(gifterId, "boinacoin") + earned;
        CPH.SetKickUserVar(gifterId, "boinacoin", balance, true);

        // ── 3. Estadística histórica ─────────────────────────
        long totalEarned = CPH.GetKickUserVar<long>(gifterId, "boinacoin_total_earned") + earned;
        CPH.SetKickUserVar(gifterId, "boinacoin_total_earned", totalEarned, true);

        // ── 4. Timestamp antiinactividad ─────────────────────
        CPH.SetKickUserVar(gifterId, "boinacoin_last_seen",
